 ...
 *
 * A ComponentScope is created once per node, at the point that node starts
 ...
 *
 * carries, at minimum:
 *   - nodeId      — the topology node currently in control (see SPEC.md §3.6)
 *   - componentId — the component that node is an instance of
 *   - classLoader — the thread context classloader this node's code
 *                   should run under (see ItaraClassLoader)
 *
 * This list is expected to grow. Construction goes through the nested
 ...
 *
 * The currently-active scope is held in an InheritableThreadLocal, not a
 ...
 *
 * There is no explicit stack (ADR 0022) — only the single, currently-active
 ...
 * already nests.
 *
 * Setting the active scope is restricted to this package — only
 * ComponentScopeHandle may do it. Component code, and everything outside
 * io.itara.runtime, may only read the active scope via current().
 */
