 ...
 *
 * get(targetIdentifier, type) — resolve a dependency by its target
 ...
 *
 * getSelf(componentId, type) — for standalone entry-point code (a bespoke
 ...
 * io.itara.api is for interfaces component authors implement, not this.
 * Left in io.itara.runtime for now since most of the package structure
...
public final class ComponentLookup {
     ...
     *
     * The returned proxy is safe to call any number of times, from any
     * thread, for the life of the process — each call opens componentId's
     * own ComponentScope fresh, around just that call.
     *
     * For standalone entry-point code only (a bespoke main(), a one-off
     ...
     * meaningful in the first place. Package-private: nothing outside
     * io.itara.runtime should be disabling this for anyone else.
     */
    static void disable() {
        CLAIMED.set(true);
    }
