 ...
 * cover on its own.
 *
 * InheritableThreadLocal (see ComponentScope) is complete and sufficient
 ...
 * correct.
 *
 * Each wrap() call captures ComponentScope.current() at the moment it is
...
public final class ComponentScopePropagation {
     ...
     * individually, per call. The returned ExecutorService delegates
     * everything else (lifecycle: shutdown/awaitTermination/etc.) straight
     * through to the underlying executor, which this utility does not own
     * and does not affect the lifecycle of.
     */
