...
public class InventoryDiagnosticBean {
    ...
    public InventoryDiagnosticBean() {
        ...
        log.info("[SPIKE][LAZY-INIT] InventoryDiagnosticBean constructing on thread={} tccl={}",
                thread.getName(), tccl);

        try {
            Class<?> loaded = tccl.loadClass("com.example.inventory.InventoryService");
            this.loadedMarkerClassName = loaded.getName();
            log.info("[SPIKE][LAZY-INIT] loadClass succeeded, resolved={} loadedBy={}",
                    loaded.getName(), loaded.getClassLoader());
        } catch (ClassNotFoundException e) {
            log.error("[SPIKE][LAZY-INIT] loadClass FAILED under tccl={}", tccl, e);
