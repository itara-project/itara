 ...
 *
 * <p>Every serializer must be able to handle the error payload unconditionally,
 ...
 *
 * <p>This is deliberately not tied to any message format's own wire
 ...
 *
 * <p>Wire format — a sequence of fields, each:
 * <ul>
 * <li>1 byte — tag</li>
 * <li>1 byte — null-flag (0 = null, nothing more follows for this field;
 * 1 = present, length + value follow)</li>
 * <li>4 bytes — length, big-endian signed int, UTF-8 byte count; only
 * present when null-flag = 1</li>
 * <li>N bytes — value, UTF-8 bytes; only present when null-flag = 1</li>
 * </ul>
 *
 * <p>Every field, including the two that are never actually null in practice
 ...
 * <p>Current tags:
 * <ul>
 * <li>1 = errorKind (ErrorKind.name(), not ordinal — see below)</li>
 * <li>2 = remoteExceptionClass</li>
 * <li>3 = message (nullable)</li>
 * </ul>
 *
 * <p>Forward/backward compatibility:
 *
 * <p>Decoding never fails because of an unrecognized tag. Every field is
 ...
 * fail on.
 *
 * <p>errorKind is encoded by name, not ordinal, so a reordering of the
