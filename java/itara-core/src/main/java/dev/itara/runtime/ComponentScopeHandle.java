 ...
 *
 * <p>open() captures whatever scope (and TCCL) was previously active on this
 ...
 * }</pre>
 *
 * <p>There is no explicit stack. Because every crossing point pairs its own
 ...
 *
 * <p>A ComponentScopeHandle is deliberately much thinner than ItaraScope: no
 ...
 *
 * <p>close() is safe to call more than once — it re-applies the same
 * previously-captured values each time — though normal use (try-with-
 * resources) never does this.
 *
 * <p>ComponentScope itself stays immutable: this class holds the previous
...
public final class ComponentScopeHandle implements AutoCloseable {
     ...
     *
     * <p>Captures the thread's actual current context classloader directly
     ...
     * @return a handle that restores the previously-active scope and
     *         classloader when closed
     */
