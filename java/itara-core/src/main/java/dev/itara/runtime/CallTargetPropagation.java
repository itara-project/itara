 ...
 * headers — the agent-owned mechanism behind the call target being
 * conveyed independently of the serialized payload (§7.5, §15.6, §16.5).
 * Mirrors {@link ContextPropagation}'s role for ItaraContext: an entirely
 * separate concern with its own header keys, merged into the same
 * outbound header map but never confused with it.
 *
 * <p>A transport's own routing mechanism (e.g. an HTTP path) may also happen
...
public final class CallTargetPropagation {
     ...
     *
     * @param target the call target to serialize
     ...
     * @return the propagated method name, or null if absent
     */
